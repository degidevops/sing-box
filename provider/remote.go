...
// OutboundBuilder is the outbound builder.
type OutboundBuilder func(ctx context.Context, router adapter.Router, logger log.ContextLogger, options option.Outbound) (adapter.Outbound, error)
...
func NewRemote(ctx context.Context, router adapter.Router, builder OutboundBuilder, logger log.ContextLogger, logFactory log.Factory, options option.Provider) (*Remote, error) {
	if options.Tag == "" {
...
func (s *Remote) Update() error {
	...
	content, hash, err := s.downloadWithCache()
	if err != nil {
		return err
	}
	...
	s.loadedHash = hash
	opts, err := s.getOutboundsOptions(content)
	if err != nil {
		return err
	}
...
func (s *Remote) downloadWithCache() ([]byte, string, error) {
	content, err := s.download()
	if err == nil {
		hash := contentHash(content)
		if s.loadedHash != hash && s.cacheFile != "" {
			s.logger.Debug("write to cache file")
			os.WriteFile(s.cacheFile, content, 0o666)
		}
	...
	// load from cache
	s.logger.Warn("loading cache [", s.cacheFile, "] due to: ", err)
