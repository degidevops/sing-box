...
func New(
	...
	if options.Interval <= 0 {
		options.Interval = option.Duration(C.DefaultURLTestInterval)
	}
...
func (h *HealthCheck) ReportFailure(tag string) {
	history := h.Storage.Latest(tag)
	if history == nil || history.Delay != Failed ||
		// don't put too many failed records, which interferes with the max_fail assertion
		time.Since(history.Time) > time.Duration(h.options.Interval) {
